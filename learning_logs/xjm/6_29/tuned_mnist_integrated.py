...
# DataLoaders for Flow Matching model
train_dataloader_fm = DataLoader(
    train_data,
    batch_size=BATCH_SIZE_FM,
    shuffle=True
)
test_dataloader_fm = DataLoader(
    test_data,
    batch_size=BATCH_SIZE_FM,
    shuffle=False
)
...
# --- 2. Sample and Distribution Classes (from Flow Matching code) ---
class Sample(ABC):
...
# --- 3. ODE and Simulator Classes (from Flow Matching code) ---
class ODE(ABC):
...
# --- 4. VectorFieldPredictor (Network) and Training Functions (from Flow Matching code) ---
...
def test_step_cl(data_loader:DataLoader, model, device, loss_fn):
    ...
    with torch.inference_mode():
        for i, (test_features_batch, test_labels_batch) in enumerate(tqdm(data_loader, desc="Testing CL", leave=False)):
...
optimizer_cl = torch.optim.SGD(params=model_CL.parameters(), lr=0.1)

EPOCHS_CL = 5 # Can increase if needed for better performance, but 3 is usually good for MNIST
...
class Simulator_guided(Simulator):
    ...
    def simulate(self, steps: int, device: str = 'cpu'):
        ...
        for i, t_val in enumerate(tqdm(ts, desc=f"Generating digit {self.target_digit}", leave=False)):
            # Defensive check: Ensure X requires gradients at the start of EACH iteration
            # This handles any potential state loss from previous operations (e.g., detach())
            ...
            target_logit = logits[:, self.target_digit]
            ...
            v_guided_flat = v_flow_flat + self.guidance_scale * guidance_grad_flat
...
guidance_strength = 3.0 # This is a crucial hyperparameter to tune (e.g., 0.5 to 5.0)
...
num_samples_to_generate_guided = 5
...
# It's important to keep the outer loop within torch.no_grad() but allow gradients for X inside simulate
...
# Visualize some of the verified images
